...
type PostFeedQueryData = InfiniteData<PostsPage>;
...
export function useSubmitPostMutation() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (newPost) => {
      ...
      const predicateFn = (query: PostFeedQuery) => {
        return (
          query.queryKey.includes(endpoint) ||
          (query.queryKey.includes("user-posts") &&
            query.queryKey.includes(user!.id))
      ...
      const debouncedQueries = setTimeout(() => {
        queryClient.setQueriesData<PostFeedQueryData>(
          queryFilter,
          (oldData) => {
            if (!oldData) return undefined;
        ...
        queryClient.invalidateQueries({
          queryKey: queryFilter.queryKey,
          predicate: (query: PostFeedQuery) =>
            predicateFn(query) && !query.state.data,
        });
      }, 0);
